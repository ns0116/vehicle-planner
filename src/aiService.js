...
const LAYER_INSTRUCTIONS = [
  "Layer 1: 市場調査 (Market Research)\n指定されたセグメント・ボディタイプのグローバル市場構造を解剖し、TAM/SAM/SOMおよびCAGRを定量的に評価せよ。表面的なトレンドの羅列を排し、マクロ経済動向、購買力シフト、人口動態の変化を掛け合わせたエコノミクス分析から、市場の真の成長ドライバーと潜在的リスク（市場縮小・コモディティ化の予兆）を抽出・洞察せよ。",
  "Layer 2: 法規制調査 (Regulation Research)\nPEST分析における「Political」の観点から、対象市場の環境規制（排出ガス・燃費・LCA）、安全基準、地政学的リスク、補助金政策の最新動向を網羅せよ。これらを単なる制約条件としてではなく、「ルール形成による競争優位（Regulatory Moat）」や「インフラ整備（充電網等）がもたらす参入障壁」という戦略的変数として評価し、事業の成立要件を定義せよ。",
  ...
  "Layer 4: ユーザー調査 (Persona Setup)\n3C分析の「Customer」視点から、表層的なライフスタイル描写を排除せよ。ターゲット顧客の深層心理にあるペインポイント、真のJobs-to-be-Done（片付けるべき用事）、およびWillingness to Pay（支払意欲）を冷徹に分析し、高解像度のペルソナを構築せよ。既存製品では満たされていないホワイトスペース（Unmet Needs）を明確に炙り出せ。",
  ...
  "Layer 7: 自社アセット・ブランド分析 (Company Analysis)\n3Cの「Company」視点およびVRIOフレームワーク（価値、希少性、模倣困難性、組織）を用い、指定された自社ブランド（OEM）のコア・コンピタンスを評価せよ。単なる過去の遺産や主観的なブランドイメージではなく、対象セグメントの苛烈な競争環境において、他社が構造的に追随できない『勝つための必然性（Right to Win）』と『強固な経済的堀（Moat）』を厳密に定義せよ。",
