...
## 🧠 The 12-Layer Thought Architecture

単なるChatGPTのラッパーアプリからの脱却を図り、プロフェッショナルな商品企画職と同等の深さで思考させるため、AIの推論プロセスを**12のレイヤー（階層）**に分割し、直列で実行させるアーキテクチャを採用しています。

前半（Layer 1〜5, 7）は**トップコンサルタント（マーケティング）**の視点で徹底的なPEST/3C分析を行い、中盤（Layer 6, 8〜10）は**世界最高峰のデザインファーム（デザイン思考）**の視点で非常識なインサイトを生み出します。そして終盤（Layer 11〜12）は**悪魔の代弁者（Red Teamer）とデータサイエンティスト**による批判的再構築と可視化を行います。
...
### 🛡️ フェーズ3：監査と自己修正・可視化（Red Teamer & Data Visualizer）
*Designed by: AI Architect (Antigravity)*

* **Layer 11: 悪魔の代弁者による監査 (Red Team Audit) - 🔍 Dynamic Web Search**
  * これまでの自らのアイデアに対し、リアルタイム検索を用いて「粗探し」を実行。技術的な実現不可能さ、コスト矛盾、競合に対する優位性の甘さを容赦なく批判し、致命的な欠陥を暴き出す。
* **Layer 12: デザイン思考による再構築と可視化 (Radical Refinement & Visualization)**
  * 監査役からの痛烈な批判を受け止め、コンセプトを自己修正ループによってさらに強靭なものへ練り直す。最終的に競合（テスラ等）との比較をレーダーチャートとして描画するためのJSONデータを生成する。
